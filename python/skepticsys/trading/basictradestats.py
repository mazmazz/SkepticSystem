...
class BasicTradeStats(Analyzer):
    ...
    params = (
            ...
            # Use standard dict instead of AutoOrderedDict
            ('useStandardDict', False)
            )
    ...
    def create_analysis(self):
        ...
        o = self.rets = self.get_dict()  # Return user object..
        # Stats applied to all trades (winners and losers)..
        o['all'] = self.get_dict()
        o['all']['trades'] = self.get_dict()
        o['all']['pnl'] = self.get_dict()
        o['all']['streak'] = self.get_dict()
        o['all']['stats'] = self.get_dict()
        ...
        for each in ['won', 'lost']:
            oWL=self.rets[each] = self.get_dict()
            oWL['trades'] = self.get_dict()
            oWL['pnl'] = self.get_dict()
            oWL['streak'] = self.get_dict()
    ...
    def calculate_statistics(self):
        ...
        if self._all_pnl_list!=[]:
            ...
            ## TODO ## convert math operations for standard dict
